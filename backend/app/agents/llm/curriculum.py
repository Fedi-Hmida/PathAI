...
def _build_schema_focused_prompt(payload: CurriculumAgentInput) -> str:
    ...
    return (
        ...
        "Each week must include ALL of: week_id, week_number, theme, topics, "
        "estimated_hours, learning_outcomes.\n"
        "Week-level value rules:\n"
        "- week_id: must start with 'week_' followed by letters/digits/underscores, e.g. "
        "'week_1_foundations'.\n"
        "- week_number: integer from 1 upward.\n"
        "- topics: a non-empty list (max 8) of CurriculumTopicDTO.\n"
        "- estimated_hours: a number greater than 0 and at most 80.\n"
        "- learning_outcomes: 1-12 short strings.\n"
        "Each topic must include ALL of: topic_id, title, description, concept_ids, "
        "difficulty, estimated_hours, learning_outcomes, sequence_order.\n"
        "Strict value rules (output is rejected otherwise):\n"
        "- topic_id: must start with 'topic_' followed by letters/digits/underscores, e.g. "
        "'topic_tokenization_basics'.\n"
        "- concept_ids: 1-12 lowercase snake_case ids matching ^[a-z][a-z0-9_]+$, e.g. "
        "'word_embeddings' (never display labels like 'Word Embeddings').\n"
        "- difficulty: exactly one of 'beginner', 'intermediate', 'advanced'.\n"
        "- estimated_hours: a number greater than 0 and at most 40.\n"
        "- learning_outcomes: 1-8 short strings.\n"
        "- sequence_order: an integer from 1 upward, unique within the curriculum.\n"
        "- target_outcomes: a non-empty list of short strings."
    )
